    ...
    try:
        async with aiosqlite.connect(DB_FILE) as db:
            ...
            if row:
                ...
                songs_found = (songs_found or 0) + 1
                new_top_artist = artist or top_artist
                try:
                ...
                await db.execute("""
                    UPDATE users SET songs_found=?, top_artist=?, last_song=?, last_active=?, genres_json=? WHERE user_id=?
                """, (songs_found, new_top_artist, title, now, json.dumps(genres, ensure_ascii=False), user_id))
            else:
                genres = {}
                if genre:
                    genres[genre] = 1
